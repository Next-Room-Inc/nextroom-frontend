  ...
  useEffect(() => {
    const verify = async () => {
      ...
      try {
        ...
        // navigate to student dashboard after user logged in 
        if (!res.error) {
          toast.success('Your email has been verified successfully. You may now log in.')
    ...
    <StudentSignupLayout>
      ...
      {isSuccess && !loading && (
        <div className="text-center">
          ...
          <motion.button
            type="button"
            onClick={() => navigate(ROUTES.BASE_PATH)}
            className={buttonClass}
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
            transition={{ type: "spring", stiffness: 300 }}
