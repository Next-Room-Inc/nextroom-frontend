...
export const PropertyCard: React.FC<{
  ...
  const likedPropertyHandler = () => {
    ...
    toast.info(
      updatedState
        ? "Demo: Added to Favourites"
        : "Demo: Removed from Favourites"
    );
  };
    ...
    <>
      <PulseHoverLayout>
        <div
          ...
          className={`  cursor-move z-10 md:flex ${
            selected ? "rounded-tr-xl rounded-tl-xl " : "rounded-xl"
          } shadow-md cursor-none overflow-hidden relative p-6 text-white ${bgClass}`}
        >
          {/* Like Icon */}
          {/* <motion.img
      ...
      {selected && viewDetails && (
        <AvailableUnitsModal {...{ floorplan, property }} />
      )}
    </>
  );
};
