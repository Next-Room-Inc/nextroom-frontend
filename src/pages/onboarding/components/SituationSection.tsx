...
const HomeSection: React.FC<{
  ...
  return (
    <div className="text-center mt-35">
      <p className="text-3xl text-[#B3322F] px-10">
        Let's discuss some possible situations, <br />{" "}
        <span className="font-bold">{name}</span>
      </p>
      <img
        alt=""
        className="h-50 -my-5 pr-1 mx-auto "
        src="/assets/img/icons/situationicon.svg"
      />
      <p className="text-md pb-10">
        Please answer these to <br />
...
const DisruptionHandlingSection: React.FC<{
  ...
  const disabled =
    answers.impairedRoommate === null || answers.lightsOn === null;
  ...
  return (
    <>
      ...
      >
        ...
        <div className="flex flex-col md:flex-row gap-5 md:gap-15 justify-center items-center mt-15 text-md px-10">
          {[
            ...
            <PrimaryButton
              selected={answers.impairedRoommate === btn.value}
              onClick={() =>
                handleAnswer(sectionName, "impairedRoommate", btn.value)
              }
            >
              {btn.name}
            </PrimaryButton>
      ...
      >
        {/* Question */}
        <QuestionTitle>
          What would you do if they left the lights on, and you were sharing
          utilities?
        </QuestionTitle>
        <div className="flex flex-col md:flex-row gap-5 md:gap-15 justify-center items-center mt-15 text-md px-10">
          {[
            ...
            <PrimaryButton
              selected={answers.lightsOn === btn.value}
              onClick={() => handleAnswer(sectionName, "lightsOn", btn.value)}
            >
              {btn.name}
            </PrimaryButton>
          ))}
...
const ConflictAndSafetySection: React.FC<{
  ...
  const disabled =
    answers.illicitSubstances === null ||
    answers.verbalPhysicalAltercation === null;
      ...
      >
        ...
        <div className="flex flex-col md:flex-row gap-5 md:gap-15 justify-center items-center mt-15 text-md px-10">
          {[
            ...
            <PrimaryButton
              selected={answers.illicitSubstances === btn.value}
              onClick={() =>
                handleAnswer(sectionName, "illicitSubstances", btn.value)
              }
            >
              {btn.name}
            </PrimaryButton>
      ...
      >
        ...
        <div className="flex flex-col md:flex-row gap-5 md:gap-15 justify-center items-center mt-15 text-md px-10">
          {[
            ...
            <PrimaryButton
              selected={answers.verbalPhysicalAltercation === btn.value}
              onClick={() =>
                handleAnswer(
                  sectionName,
                  "verbalPhysicalAltercation",
                  btn.value
                )
              }
            >
    ...
    <>
      ...
      >
        ...
        <div className="flex flex-col md:flex-row gap-5 md:gap-15 justify-center items-center mt-15 text-md px-10">
          {[
            ...
            <PrimaryButton
              selected={answers.loudParties === btn.value}
              onClick={() =>
                handleAnswer(sectionName, "loudParties", btn.value)
              }
            >
              {btn.name}
            </PrimaryButton>
      ...
      >
        ...
        <div className="flex flex-col md:flex-row gap-5 md:gap-15 justify-center items-center mt-15 text-md px-10">
          {[
            ...
            <PrimaryButton
              selected={answers.owedMoney === btn.value}
              onClick={() => handleAnswer(sectionName, "owedMoney", btn.value)}
            >
              {btn.name}
            </PrimaryButton>
          ))}
      ...
      {answers.owedMoney !== null && answers.loudParties !== null && (
        <>
          <img
            alt=""
            className="h-15 mt-20 mb-5 pr-1 mx-auto "
          ...
          <p className="px-10">
            <span className="font-bold">
              {" "}
              It’s important to take these situations seriously.{" "}
            </span>
            Simply avoiding or not addressing <br />
            an issue puts yourself and others at risk. Use our reporting tools
            if you need help. <br />
            If the situation puts you in immediate danger,
            <span className="font-bold"> always call 911 first.</span>
          </p>
        </>
      )}
...
const AllSetSection: React.FC<{
  ...
  return (
    <>
      <img
        alt=""
        className="h-35 my-5 pr-1 mx-auto "
        src="/assets/img/icons/owlflight2.svg"
      />
      {/* Question */}
      ...
      <PrimaryButton
        ...
        selected={true}
        className="mx-auto bg-black mt-10 border"
      >
