  ...
  return (
    <>
      ...
      <div className="text-center mt-35">
        <p className="text-3xl text-[#B3322F]">
          Welcome To Next Room <br /> <span className="font-bold">{name}</span>
        </p>

        <img
          alt=""
          className="h-40 pr-1 mx-auto -my-4"
          src="/assets/img/icons/survey_icon.svg"
        />

        <p className="text-md">
          {" "}
          Let's Find Your Perfect Home. <br /> We'll Ask A Few Questions To Get
          Started{" "}
        </p>
        ...
        <div className="flex flex-col md:flex-row gap-6 justify-center items-center mt-12 text-md">
          ...
          <PrimaryButton
            button={true}
            onClick={() =>
              updateOnboardingStatusHandler({ onboardingFormSkipped: true })
            }
          >
...
const AmenitiesSection: React.FC<{
  ...
  const [selectedUnitAmenities, setSelectedUnitAmenities] = useState<string[]>(
    answers.unitAmenities || []
  );
  const [selectedComunityAmenities, setSelectedComunityAmenities] = useState<
    string[]
  >(answers.communityAmenities || []);

  const handleUnitAmenityToggle = (amenity: string) => {
    setSelectedUnitAmenities(
      (prev: string[]) =>
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 px-8 mt-6  mx-auto max-w-fit   ">
          {unitAmenities.map((amenity, index) => (
            <PrimaryButton
              key={index}
              selected={selectedUnitAmenities.includes(amenity.name)}
              onClick={() => handleUnitAmenityToggle(amenity.name)}
            >
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 px-8 mt-6  mx-auto max-w-fit">
          {comunityAmenities.map((amenity, index) => (
            <PrimaryButton
              key={index}
              selected={selectedComunityAmenities.includes(amenity.name)}
              onClick={() => handleCommunityAmenityToggle(amenity.name)}
            >
              <div className="flex items-center w-full gap-2 px-4">
                <div className="flex-shrink-0 w-6 h-6">
                  <img
...
const RoommatesSection: React.FC<{
  ...
  const increment = () => {
    handleAnswer("PROPERTY_SECTION", "roommateCount", count + 1);
  };
  const decrement = () => {
    if (count > 0) handleAnswer("PROPERTY_SECTION", "roommateCount", count - 1);
  };
  ...
  return (
    <>
      ...
      >
        ...
        <div className="flex flex-col md:flex-row gap-6 justify-center items-center mt-15 text-md px-10">
          <PrimaryButton
            ...
            onClick={() =>
              handleAnswer("PROPERTY_SECTION", "wantsRoommates", true)
            }
          >
            {" "}
            Yes{" "}
      ...
      {answers.wantsRoommates && (
        <>
          ...
          >
            <p className="text-2xl text-[#B3322F]   font-semibold w-full  px-10 text-center mx-auto">
              {" "}
              Do you want help finding additional roommates?{" "}
            </p>
            <div className="flex flex-col md:flex-row gap-6 justify-center items-center mt-15 text-md px-10">
              <PrimaryButton
          ...
          {!answers?.wantsRoommateMatching ? (
            <div>
              <p className="text-2xl text-[#B3322F] mt-10 font-semibold w-full  px-10 text-center mx-auto">
                {" "}
      ...
      <NextButton
        onClick={disabled ? scrollHandler : nextHandler}
        previousStepHandler={previousStepHandler}
      />
    </>
  );
...
const LookingForSection: React.FC<{
  ...
  const [rentalDescription, setRentalDescription] = useState<string | null>(
    null
  );
  return (
    <>
      ...
      >
        ...
        <div className="flex flex-col md:flex-row gap-6 justify-center items-center mt-10 text-md px-10">
          {rentalTypes.map((rental) => (
            <div className="w-full relative  md:w-auto group">
              {rentalDescription === rental.name ? (
                <motion.p
                  ...
                  className={` absolute bottom-11 right-0 max-w-[200px] text-[8px] mx-auto px-2 py-2 mb-3 bg-white shadow-[#D9D9D9] drop-shadow-xl shadow-md`}
                >
                  {rental.details}
                </motion.p>
              ) : (
                <p
                  className={`absolute bottom-11 right-0 max-w-[200px] text-[8px] mx-auto px-2 py-2 mb-3 bg-white shadow-[#D9D9D9] drop-shadow-xl shadow-md opacity-0 hidden md:flex`}
                >
                  {rental.details}
                </p>
              )}
              ...
              >
                <div className="flex gap-2  w-full justify-center">
                  <img
                    alt=""
                    className="h-5 "
                    src={`/assets/img/icons/${rental.icon}`}
                  />
                  {rental.name}
                </div>
                ...
                <img
                  ...
                  onClick={() =>
                    rentalDescription === rental.name
                      ? setRentalDescription(null)
                      : setRentalDescription(rental.name)
                  }
                  className="h-6 relative right-1 mr-2 group"
                  src={`/assets/img/icons/question_circle_icon.svg`}
                />
              </Button>
            </div>
          ))}
  ...
  return (
    <>
      ...
      >
        <p className="text-2xl text-[#B3322F] font-semibold w-[60%]  md:w-[80%] text-center mx-auto">
          {" "}
          How close to campus is your preferred rental?{" "}
        </p>
...
const BudgetSection: React.FC<{
  ...
  const [range, setRange] = useState<any>([
    answers?.budgetMin ? Number(answers?.budgetMin) : min,
    answers?.budgetMax ? Number(answers?.budgetMax) : null,
  ]);

  const scrollHandler = () => {
    setError(true);
    console.log(range[1] === null, "==>", range);
    if (range[1] === null) {
  ...
  const moveToNextStepHandler = () => {
    ...
    handleAnswer("PROPERTY_SECTION", "budgetMin", minValue);
    handleAnswer("PROPERTY_SECTION", "budgetMax", maxValue);
  ...
  return (
    <>
      ...
      >
        ...
        <div className="mt-10  px-10 md:px-50">
          ...
          <Slider
            ...
            allowCross={false}
            value={range}
            trackStyle={[{ backgroundColor: "#b3322f", height: 10 }]} // green track
            handleStyle={[
              { borderColor: "#b3322f", height: 24, width: 24, marginTop: -7 },
              {
                borderColor: "#b3322f",
            ...
            railStyle={{ backgroundColor: "#b3322f", height: 10 }}
      ...
      <NextButton
        onClick={disabled ? scrollHandler : moveToNextStepHandler}
        previousStepHandler={previousStepHandler}
      />
    </>
  );
  ...
  return (
    <>
      ...
      >
        ...
        <div className="flex flex-col md:flex-row gap-6 justify-center items-center mt-20 text-md px-10">
          <PrimaryButton
            selected={answers.stayDurationMonths === "4 Months"}
            onClick={() =>
              handleAnswer("PROPERTY_SECTION", "stayDurationMonths", "4 Months")
            }
          >
            {" "}
            4 Months{" "}
          ...
          <PrimaryButton
            selected={answers.stayDurationMonths === "6 Months"}
            onClick={() =>
              handleAnswer("PROPERTY_SECTION", "stayDurationMonths", "6 Months")
            }
          >
            {" "}
            6 Months{" "}
          ...
          <PrimaryButton
            selected={answers.stayDurationMonths === "8 Months"}
            onClick={() =>
              handleAnswer("PROPERTY_SECTION", "stayDurationMonths", "8 Months")
            }
          >
            {" "}
            8 Months{" "}
          ...
          <PrimaryButton
            selected={answers.stayDurationMonths === "12 Months"}
            onClick={() =>
              handleAnswer(
                "PROPERTY_SECTION",
                "stayDurationMonths",
        ...
        >
          <input
            ...
            type="date"
            className=" input-white-calendar px-10 text-center py-2 text-white focus:outline-none   "
            ref={dateInputRef}
          />
        </div>
      </div>
  ...
  return (
    <>
      ...
      >
        ...
        <div className="flex flex-col md:flex-row gap-6 justify-center items-center mt-20 text-md px-10">
          <PrimaryButton
            selected={answers.moveInDate === "September 1"}
            onClick={() =>
              handleAnswer("PROPERTY_SECTION", "moveInDate", "September 1")
            }
          >
            {" "}
            September 1{" "}
          ...
          <PrimaryButton
            selected={answers.moveInDate === "May 1"}
            onClick={() =>
              handleAnswer("PROPERTY_SECTION", "moveInDate", "May 1")
            }
          >
            {" "}
            May 1{" "}
  ...
  return (
    <div className="text-center">
      ...
      >
        ...
        <div
          className={`  flex flex-col  md:flex-row gap-6 justify-center items-center text-md px-10 mt-10`}
        >
          {options.map(({ label, value }) => (
            <PrimaryButton
              key={value}
              onClick={() =>
                handleAnswer("PROPERTY_SECTION", "rentalType", value)
              }
